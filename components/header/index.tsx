...
export const Header = ({ menuList }: HeaderProps) => {
  ...
  return (
    <>
      ...
      fixed top-0 left-0 w-full h-full  bg-black/70 backdrop-blur-sm text-black max-w-[40rem] ${
        ...
        <ul className="pt-10 pl-10 max-h-full overflow-y-auto hide-scrollbar ">
          {menuList.map((item) => (
            <li key={item.id} className="list-item">
              <a
                href={item.link}
                className="flex items-center gap-3 text-[1.6rem] my-4 text-[#bb8866] hover:text-white"
                onClick={handleHamburger}
              >
                {item.name}
              </a>
    ...
    <ul className="pl-8">
      {item?.submenu?.map((subItem) => (
        <li key={subItem.id}>
          <a
            href={subItem.link}
            className="flex items-center gap-3  text-[#bb8866] before:bg-black text-[1.2rem] my-4 hover:text-white"
          >
            {subItem.name}
          </a>
        </li>
