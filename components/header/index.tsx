...
export const Header = ({ menuList }: HeaderProps) => {
    ...
    return (
        <header className="  px-4 py-7 flex justify-between items-center absolute w-full z-20">
            ...
            >
                <Image src='/image/nandivardhan-logo.png' alt="logo" className="w-full" width={100} height={200} />
            </a>
            <button
                onClick={handleHamburger}
                className="hamburger w-8 h-8 flex flex-col justify-center gap-2 "
            >
                <span className="w-full h-1 bg-white lg:bg-black "></span>
                <span className="w-full h-1 bg-white lg:bg-black "></span>
                <span className="w-full h-1 bg-white lg:bg-black "></span>
            </button>
      ...
      fixed top-0 left-0 w-full h-full  bg-white text-black max-w-[40rem] ${menuOpen ? " active translate-x-0" : "-translate-x-full"
